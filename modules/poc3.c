...
void dccp_kmalloc_pad() { //4个套接字连接到端口11000-11003
	int i;
...
void kmalloc_pad() { //dccp_kmalloc_pad-> timer_kmalloc_pad -> dccp_kmalloc_pad
...
void setup_sandbox() {
	...
	cpu_set_t my_set;
	CPU_ZERO(&my_set);
	CPU_SET(0, &my_set);//将第0个cpu加入到cpu集
	if (sched_setaffinity(0, sizeof(my_set), &my_set) != 0) {//绑定当前进程运行在cpu集
